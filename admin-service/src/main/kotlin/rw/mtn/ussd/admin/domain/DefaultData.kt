...
class DataLoader(
    ...
    private fun seedMenus() {
        if (menuRepo.count() > 0L) {
            val existing = menuRepo.findAll()
            var updated = false
            existing.forEach { m ->
                val shouldRequire = m.targetKey in setOf("me2u", "voicepack", "bundleinternet", "prestige")
                if (m.requiresPhone != shouldRequire) {
                    m.requiresPhone = shouldRequire
                    updated = true
                }
            }
            if (updated) {
                menuRepo.saveAll(existing)
            }
            return
        }
